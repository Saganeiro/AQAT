...
export class CustomReporter {
  ...
  static generateJSONReport(result: AuditResult, url: string): void {
    const reportData = {
      url,
      timestamp: new Date().toISOString(),
      consoleErrors: result.consoleErrors.length,
      networkErrors: result.networkErrors.length,
      largeResources: result.largeResources.length,
      brokenLinks: result.brokenLinks.length,
      accessibilityIssues: result.accessibilityIssues.length,
      performance: {
        lcp: result.performanceMetrics.lcp,
        fcp: result.performanceMetrics.fcp
      },
      visualRegressionPassed: result.visualRegressionPassed
    };
    const filePath = path.join(process.cwd(), 'reports', 'audit-report.json');
